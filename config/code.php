...
class AwsModel extends BaseModel
{
    ...
    public function putObject($key, $tempPath)
    {
        if (file_exists($tempPath)) {
            try {
                ...
                log_message('error', 'AWS S3 PutObject Error: ' . $e->getMessage());
                return false;
            }
        }
    }
    ...
    public function getObject($bucket, $key)
    {
        try {
            ...
            log_message('error', 'AWS S3 GetObject Error: ' . $e->getMessage());
            return false;
        }
        return true;
    }
    ...
    public function deleteObject($key)
    {
        if (!empty($key)) {
            try {
                ...
                log_message('error', 'AWS S3 DeleteObject Error: ' . $e->getMessage());
                return false;
            }
        }
        return true;
